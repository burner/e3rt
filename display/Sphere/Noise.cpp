...
unsigned short permu[] = { 151,160,137,91,90,15,
...
double fbm(double x, double y, double z, int oct, float lac, float gain) {
	double freq = 1.0;
	double amp = 0.5;
	double sum = 0.0;
...
double turbulence(double x, double y, double z, int oct, float lac, float gain) {
	double freq = 1.0;
	double amp = 0.5;
	double sum = 0.0;

	for(int i = 0; i < oct; i++) {
		sum += abs(snoise(x*freq, y*freq, z*freq)) * amp;
		freq *= lac;
		amp *= gain;
	}
	return sum;
